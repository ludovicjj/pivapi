...
class SearchPost
{
    ...
    {
        $posts = $this->requestLoader->load($request);
        $linkPaginator = $this->paginatorBuilder->build($posts->count())->getLink();

        $output = new OutputSearchResult(iterator_to_array($posts), $linkPaginator);
